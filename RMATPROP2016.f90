  ...
  !                   TempG = TempG + a*BPD%u(lx,kx,ix)*2*reducedmass*(Energy - (BPD%Pot(mch,mch,lx,kx) - &
                  ...
                  ENDDO
                  EIG%Gam((mch-1)*BPD%xDim+ix,(mch-1)*BPD%xDim+ixp) = EIG%Gam((mch-1)*BPD%xDim+ix,(mch-1)*BPD%xDim+ixp) &
                  + TempG ! place values into Gamma0
               ENDDO
               ...
               DO nch = 1, mch-1
                  DO kx = BPD%kxMin(ixp,ix),BPD%kxMax(ixp,ix)
                     ...
                     ENDDO
                     EIG%Gam((mch-1)*BPD%xDim+ix,(nch-1)*BPD%xDim+ixp) = EIG%Gam((mch-1)*BPD%xDim +ix, (nch-1)*BPD%xDim +ixp) &
                     + TempG ! place values into EIG%Gam
                  ENDDO

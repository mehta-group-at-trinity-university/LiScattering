!****************************************************************************************************
! This code is capable of doing the entire Rubidium alakali scatteirng calculation, but the main program has been modified to do only the
! log derivative matrix at about 40 bohr or so, so that it can be used to calculate K_sr in a MQDT calculation.
!************
...
!!$  deallocate(EVAL,EVEC,HHZ)
 ...
 !____________________________________________________________________
  ...
  NPP = 100000
  VLIM = 0d0
!  A = 39  !A = 39 for K-39, 40 for K-40
...
!   call PotassiumTriplet(A,XO,NPP,VLIM,VTriplet)
  ...
  ! Remember that zgensub.f requires angular momentum quantum numbers here to be
  ! twice the physical values to accomodate half-integer spins.
  mtot = 4
  
  call MakeHF2Basis(iRb87, sRb87, iRb87, sRb87, sym, lwave, mtot, size2)
  write(6,*) "size of the symmetrized 2-atom hyperfine basis = ", size2
  allocate(SPmat(size2,size2), TPmat(size2,size2))

  S = 0
  call MakeSTHFProj(S, iRb87, iRb87, sRb87, sRb87, hf2symTempGlobal, size2, SPmat)
  write(6,*) "The singlet projection matrix:"
  write(6,*) "-------------------------------"
  call printmatrix(SPmat,size2,size2,6)

  S = 2
  call MakeSTHFProj(S, iRb87, iRb87, sRb87, sRb87, hf2symTempGlobal, size2, TPmat)
  ...
  do iB = 1, NBgrid 
     yi(:,:)=ystart(:,:)
     call MakeHHZ2(Bgrid(iB),ARb87,ARb87,gs,giRb87,giRb87,iRb87,sRb87,iRb87,sRb87,hf2symTempGlobal,size2,HHZ2)

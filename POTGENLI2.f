...
c cc Define variables for the four Li2 states of interest
...
c -----------------------------------------------------------------------
c c    ISTATE= 4      % a choice of ISTATE required when using with LEVEL
c -----------------------------------------------------------------------
c cc Specify parameter values for one of the four Li2 states of interest
...
c -----------------------------------------------------------------------
c c        READ(5,*) IPOTL, PPAR, QPAR, NSR, NLR, IBOB
c c        READ(5,*) DSCM, REQ, Rref
c c        IF(IPOTL.GE.4) THEN
c ** For MLR, DELR or Tiemann-polynomial potentials .....
c c            READ(5,*) NCMM, IVSR, IDSTT, rhoAB
c c            READ(5,*) (MMLR(I), CMM(I),I= 1,NCMM)
c c            ENDIF
c -----------------------------------------------------------------------
          PPAR= PPARA(ISTATE)
          NLR= NLRA(ISTATE)
          ...
          DO  I= 1,IORD+1
              ...
              ENDDO
c -----------------------------------------------------------------------
c c        IF(NVARB.GT.0) READ(5,*) (PARM(I), I=1,IORD+1)
c -----------------------------------------------------------------------
          PNA= 3
          NT1= -1
...
c *** Set mass numbers for reference isotopologue
          ...
          QAD= PAD
c -----------------------------------------------------------------------
c c            READ(5,*) MN1R, MN2R, PAD, QAD, NU1, NU2, PNA, NT1, NT2
c -----------------------------------------------------------------------
...
c   Initialize interaction matrix to 0.d0
      DO  I= 1,3
          H(I,I)=0.0D0
          ENDDO
c cccc Prepare interation matrix  H 
      ...
      H(3,2)= H(2,3)
      H(3,3)= DELTAE
c ccccc Call subroutine to prepare and invert interaction matrix  H
      CALL ZHEEVJ3(H,Q,W)
      L=1
c cc Nor - identify the lowest eigenvalue of  H  and label it  L
      DO J=2,3
          IF (W(J) .LT. W(L)) THEN
              L=J
              ENDIF
...
c  This loop can be omitted if only the eigenvalues are desired ---
              ...
              ENDDO
          ENDDO
c c    PRINT *, "ZHEEVJ3: No convergence.   I=", I
...
C oxon   DATA (zm(31,i),i=0,2)/69.723d0, 68.925581d0, 70.9247073d0/
